...
describe("widgets/airtrail/component", () => {
  ...
  it("displays distance in km by default", () => {
    useWidgetAPI.mockReturnValue({ data: STATS_RESPONSE, error: undefined });

    const service = { widget: { type: "airtrail", fields: ["distance"] } };
    const { container } = renderWithProviders(<Component service={service} />, { settings: { hideErrors: false } });

    expect(container.querySelector(".service-block").textContent).toContain("km");
    expect(container.querySelector(".service-block").textContent).not.toContain("mi");
  });

  it("displays distance in miles when imperial is true", () => {
    useWidgetAPI.mockReturnValue({ data: STATS_RESPONSE, error: undefined });

    // 12345.6 km * 0.621371 = ~7671 mi
    const service = { widget: { type: "airtrail", fields: ["distance"], imperial: true } };
    const { container } = renderWithProviders(<Component service={service} />, { settings: { hideErrors: false } });

    expectBlockValue(container, "airtrail.distance", 7671);
    expect(container.querySelector(".service-block").textContent).toContain("mi");
    expect(container.querySelector(".service-block").textContent).not.toContain("km");
  });
