...
export default function Component({ service }) {
  ...
  const imperial = widget.imperial ?? false;
  const distance = imperial ? Math.round(stats.distanceKm * 0.621371) : Math.round(stats.distanceKm);
  const distanceUnit = imperial ? "mi" : "km";

  return (
    <Container service={service}>
      <Block label="airtrail.flights" value={t("common.number", { value: stats.flights })} />
      <Block label="airtrail.distance" value={`${t("common.number", { value: distance })} ${distanceUnit}`} />
      ...
      <Block
        label="airtrail.topAirline"
        value={stats.topAirline ? `${stats.topAirline.name} (${stats.topAirline.count})` : "-"}
      />
      <Block
        label="airtrail.topAirport"
        value={stats.topAirport ? `${stats.topAirport.iata ?? stats.topAirport.icao} (${stats.topAirport.count})` : "-"}
      />
      <Block
        label="airtrail.topAircraft"
        value={stats.topAircraft ? `${stats.topAircraft.name} (${stats.topAircraft.count})` : "-"}
      />
      <Block
